...
NSE <- getSymbols.yahoo("^NSEI", from = "2020-11-02", to = "2023-10-26", verbose = F, auto.assign = F, periodicity = "weekly")
NSE <- na.omit(NSE)
...
T_Bill_W <- read_excel("T_Bills.xlsx",sheet = "W")
T_Bill_W <- as.xts(T_Bill_W)
head(T_Bill_W)
...
Excess_Return_NSE <-dailyReturn(NSE$NSEI.Close)[-1,]- T_Bill_W[-1,]
